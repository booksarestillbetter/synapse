...
| **`synapse-engine`** | `crates/synapse-engine` | Multi-torrent `SwarmEngine`: Hot/Warm/Cold swarm virtualization, Endgame mode, token-bucket rate limits, QueueManager, PCP/NAT-PMP/UPnP, and native post-download lifecycle hooks (hardlinking, completion scripts, instructions webhook). |
