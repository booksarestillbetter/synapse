...
async fn accept_rejects_an_unrecognized_info_hash() {
    ...
    let accept_task = tokio::spawn(async move {
        ...
        synapse_engine::accept(stream, peer_addr, [1u8; 20], move |h| h == known_hash, false, tx).await
    });
    ...
    assert!(accept_result.is_err(), "accept should reject an unknown info hash");
...
async fn connect_rejects_a_mismatched_response_hash() {
    ...
    let (tx, _rx) = mpsc::channel(1);
    let result = synapse_engine::connect(addr, [6u8; 20], requested_hash, false, tx).await;
    assert!(result.is_err(), "connect must reject a reply for the wrong info hash");
    server.await.unwrap();
}
...
async fn handshake_clears_dht_bit_for_private_swarm() {
    ...
    let server = tokio::spawn(async move {
        ...
        if let synapse_wire::Message::Handshake { reserved, .. } = msg {
            ...
            assert_eq!(reserved[7] & 0x01, 0, "DHT bit must be cleared for private swarm");
            // BEP 10 extension bit
            assert_eq!(reserved[5] & 0x10, 0x10, "BEP 10 extension protocol must be set");
    ...
    let (tx, _rx) = mpsc::channel(1);
    let res = synapse_engine::connect(addr, [0x11; 20], info_hash, true, tx).await;
    assert!(res.is_ok());
    server.await.unwrap();
}
...
async fn handshake_sets_dht_bit_for_public_swarm() {
    ...
    let server = tokio::spawn(async move {
        ...
        if let synapse_wire::Message::Handshake { reserved, .. } = msg {
            ...
            assert_eq!(reserved[7] & 0x01, 0x01, "DHT bit must be set for public swarm");
    ...
    let (tx, _rx) = mpsc::channel(1);
    let res = synapse_engine::connect(addr, [0x22; 20], info_hash, false, tx).await;
    assert!(res.is_ok());
    server.await.unwrap();
}
