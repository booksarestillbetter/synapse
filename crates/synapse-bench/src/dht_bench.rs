...
pub async fn run_dht_benchmark(iterations: usize) {
    info!("🧪 Starting DHT Subsystem Benchmark (Iterations: {})", iterations);
    ...
    for i in 0..iterations {
        ...
        let addr = format!("127.0.0.1:{}", 10000 + (i % 50000)).parse().unwrap();
    ...
    info!("✅ Routing table inserted {} nodes in {:.3}s ({:.1} ops/sec)", iterations, insert_duration.as_secs_f64(), insert_rate);
    ...
    info!("✅ Performed {} nearest-node Kademlia lookups in {:.3}s ({:.1} lookups/sec)", iterations, lookup_duration.as_secs_f64(), lookup_rate);

    // 2. DHT Live Node Ping/Pong Loopback Benchmark
    let (node_a, _addr_a) = spawn(random_node_id(), "127.0.0.1:0".parse().unwrap()).await.expect("Spawn node A");
    let (_node_b, addr_b) = spawn(random_node_id(), "127.0.0.1:0".parse().unwrap()).await.expect("Spawn node B");
    ...
    info!("✅ Completed {} UDP DHT ping roundtrips in {:.3}s ({:.1} rps, avg latency: {:.3}ms)", ping_count, ping_duration.as_secs_f64(), ping_rate, ping_avg_ms);

    println!("\n========================================================");
    println!("📊 SYNAPSE 2.0 DHT ENGINE BENCHMARK (N={})", iterations);
    println!("========================================================");
    println!("  • Routing Table Seen:    {:>10.1} ops/sec", insert_rate);
    println!("  • Kademlia Closest:      {:>10.1} lookups/sec", lookup_rate);
    println!("  • Live UDP Ping/Pong:    {:>10.1} req/sec ({:.2} ms/req)", ping_rate, ping_avg_ms);
    println!("========================================================\n");
}
