...
| **26** | HTTP/REST Tracker Protocol | Tracker | **Partial** | `synapse-tracker::bep26` | `RestTrackerClient` formats RESTful announce/scrape URLs (`/announce/{info_hash}`, `/scrape/{info_hash}`), but isn't wired into the live announce path, and the request it builds omits standard fields (`uploaded`/`downloaded`/`left`/`event`) a tracker needs to track swarm state. There is no single canonical "BEP 26" specification to conform to (unlike the other entries in this table), so this is a best-effort convention rather than a standards-verified implementation. |
