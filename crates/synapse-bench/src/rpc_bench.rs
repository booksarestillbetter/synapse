...
pub async fn run_rpc_benchmark(concurrency: usize, requests_per_worker: usize) {
    info!("🧪 Starting gRPC Control Plane Benchmark (Workers: {}, Reqs/Worker: {})", concurrency, requests_per_worker);
    ...
    for _ in 0..concurrency {
        ...
        let handle = tokio::spawn(async move {
            ...
            for _ in 0..requests_per_worker {
                let _resp = client.get_session_stats(SessionStatsRequest {}).await.expect("get_session_stats");
            }
        });
        handles.push(handle);
    }
    ...
    info!("✅ Performed {} GetSessionStats gRPC calls in {:.3}s ({:.1} req/sec, avg {:.3}ms)", total_reqs, stats_duration.as_secs_f64(), stats_rps, stats_avg_ms);
        ...
        let handle = tokio::spawn(async move {
            ...
            for req_id in 0..requests_per_worker {
                let magnet = format!(
                    ...
                    (worker_id * 100000 + req_id + 1), worker_id, req_id
                );
    ...
    info!("✅ Dispatched {} AddTorrent gRPC mutations in {:.3}s ({:.1} req/sec, avg {:.3}ms)", total_reqs, add_duration.as_secs_f64(), add_rps, add_avg_ms);

    // 3. SubscribeTorrents delta streaming latency benchmark
    let mut stream_client = SynapseControlClient::connect(endpoint.clone()).await.expect("connect");
    let stream_req = SubscribeTorrentsRequest {
        chunk_size: 500,
        flush_window_ms: 100,
    };
    let mut stream = stream_client.subscribe_torrents(stream_req).await.expect("subscribe").into_inner();
    ...
    info!("✅ Streamed initial snapshot chunks in {:.3}ms", stream_duration.as_secs_f64() * 1000.0);
    ...
    println!("  • GetSessionStats Throughput: {:>6.1} req/sec ({:.2} ms/req)", stats_rps, stats_avg_ms);
    println!("  • AddTorrent Mutation Throughput: {:>6.1} req/sec ({:.2} ms/req)", add_rps, add_avg_ms);
    println!("  • Snapshot Stream Latency: {:>8.2} ms", stream_duration.as_secs_f64() * 1000.0);
    println!("========================================================\n");
}
